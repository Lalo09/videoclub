Public Class FuncionesPelicula
    ...
    Public Sub insertar_pelicula(ByVal pelicula As ClassPelicula)
        Dim consulta As String = "INSERT INTO pelicula (id_distribuidor,nombre,duracion,genero,clasificacion,pais_origen,director,cantidad_en_existencia,sinopsis,fecha_adquicision,precio_venta,precio_renta,precio_compra,foto_caratula) VALUES ( " & pelicula.id_distribuidor & ",'" & pelicula.nombre & "','" & pelicula.duracion & "','" & pelicula.genero & "','" & pelicula.clasificacion & "','" & pelicula.pais_origen & "','" & pelicula.director & "'," & pelicula.cantidad_existencia & ",'" & pelicula.sinopsis & "','" & pelicula.fecha_adquicision & "'," & pelicula.precio_venta & "," & pelicula.precio_renta & "," & pelicula.precio_compra & ",'" & pelicula.foto_caratula & "')"
        ejecutar(consulta)
    End Sub

    Public Sub actualizar_pelicula(ByVal pelicula As ClassPelicula)
        Dim consulta As String = "UPDATE pelicula SET id_distribuidor = " & pelicula.id_distribuidor & ", nombre = '" & pelicula.nombre & "', duracion ='" & pelicula.duracion & "',genero='" & pelicula.genero & "',clasificacion='" & pelicula.clasificacion & "',pais_origen='" & pelicula.pais_origen & "',director='" & pelicula.director & "', cantidad_en_existencia = " & pelicula.cantidad_existencia & ", sinopsis = '" & pelicula.sinopsis & "', fecha_adquicision = '" & pelicula.fecha_adquicision & "', precio_venta = " & pelicula.precio_venta & ", precio_renta = " & pelicula.precio_renta & ", precio_compra = " & pelicula.precio_compra & ", foto_caratula = '" & pelicula.foto_caratula & "' WHERE id_pelicula = " & pelicula.id
        ejecutar(consulta)
    End Sub
    ...
    Public Function getNombre_pelicula(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "SELECT nombre FROM pelicula WHERE id_pelicula = '" & id & "'"
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0).ToString
                dr.Close()
    ...
    Public Function getPrecio_pelicula(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "SELECT precio_venta FROM pelicula WHERE id_pelicula = '" & id & "'"
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0).ToString
                dr.Close()
    ...
    Public Function getPrecioRenta_pelicula(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "SELECT precio_renta FROM pelicula WHERE id_pelicula = '" & id & "'"
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0).ToString
                dr.Close()
