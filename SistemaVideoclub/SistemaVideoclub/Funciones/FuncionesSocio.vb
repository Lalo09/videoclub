...
Public Class FuncionesSocio
    ...
    Public Sub dar_baja_socio(ByRef id_socio As Integer)
        Dim consulta As String = "UPDATE socio SET estatus = 'Baja' WHERE id_socio = " & id_socio
        ejecutar(consulta)
    End Sub

    Public Sub dar_alta_socio(ByRef id_socio As Integer)
        Dim consulta As String = "UPDATE socio SET estatus = 'Vigente' WHERE id_socio = " & id_socio
        ejecutar(consulta)
    End Sub
    ...
    Public Function getNombre_socio(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "SELECT cliente.nombre +' '+ cliente.apellido_paterno +' '+ cliente.apellido_materno as 'Nombre' FROM cliente, socio WHERE cliente.id_cliente = socio.id_cliente and socio.id_socio = " & id
    ...
    Public Function getUltima_renta(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "select top 1 fecha_devolucion from renta where id_socio = " & id & " order by id_renta desc"
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0)
                dr.Close()
    ...
    Public Function getnumeroUltimo_adeudo(ByRef id As Integer)
    ...
    Public Function getEstatus_Adeudo(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "select top 1 adeudo.estatus from adeudo, renta where adeudo.id_renta = renta.id_renta and renta.id_socio = " & id & " order by id_adeudo desc"
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0)
                dr.Close()
    ...
    Public Function getEstatus_Vigente(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "select estatus from socio where id_socio = " & id
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0)
                dr.Close()
    ...
    Public Function getFechaVencimiento(ByRef id As Integer)

        Try
            Conectar()
            Dim consulta As String = "SELECT fecha_vigencia FROM socio WHERE id_socio = " & id
            consultar_elemento(consulta)
            If dr.Read Then
                Return dr(0)
                dr.Close()
